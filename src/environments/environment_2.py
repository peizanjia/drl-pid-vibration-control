...
class PIDControlEnvironment:
    ...
    def reset(self, initial_state=None):
        ...
        if initial_state is not None:
            ...
            #     # 初始位移
            #     self.state_space.X[:, 0] = [np.random.uniform(-1, 1), 0, 0, 0]
            # else:
            #     # 初始速度
            #     self.state_space.X[:, 0] = [0, 0, np.random.uniform(-1, 1), 0]
    ...
    def _update_state(self):
        ...
        normalized_time = float(self.current_step / self.episode_length)
        if self.current_step == 0:
            Y = float(self.state_space.Y[0])
            normalized_time = float(0)
        else:
            Y = float(self.state_space.Y[self.current_step - 1])
            normalized_time = float((self.current_step - 1) / self.episode_length)
    ...
    def step(self, action):
        ...
        if self.current_step < self.episode_length - 1:
            # 计算当前步的控制力和输出
            X_col = self.state_space.X[:, self.current_step].reshape(4, 1)
            ...
            # self.state_space.e = -2 * X_col[0] + 2 * X_col[1]
    ...
    def _calculate_reward(self, output, control_force, pid_params):
        ...
        # 输出变化惩罚（鼓励平滑的输出变化）
        param_change = self.state_space.ed ** 2
    ...
    def get_performance_metrics(self):
        ...
        return {
            ...
            'cumulative_control_effort': self.cumulative_control_effort * self.config.CONTROL_WEIGHT,
